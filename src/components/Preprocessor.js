...
class Preprocessor {
      ...
      tap(new ExtractTextPlugin(outputPath), extractPlugin => {
        let loaders = [
...

          {
            ...
            options: {
              sourceMap: preprocessor.type === 'sass' && Config.processCssUrls ? true : Mix.isUsing('sourcemaps'),
              ident: 'postcss',
              plugins: (function() {
        ...
        this.extractPlugins = (this.extractPlugins || []).concat(extractPlugin)
      })
    })
  ...
  preprocess(type, src, output, pluginOptions = {}) {
    ...
    output = this.normalizeOutput(new File(output), src.nameWithoutExtension() + '.css')
