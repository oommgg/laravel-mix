...
class Dependencies {
  ...
  execute(command, abortOnComplete) {
    console.log('Additional dependencies must be installed. ' + 'This will only take a moment.')

    process.execSync(command)

    if (abortOnComplete) {
      console.log(typeof abortOnComplete === 'string' ? abortOnComplete : 'Finished. Please run Mix again.')
