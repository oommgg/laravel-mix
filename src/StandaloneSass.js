...
class StandaloneSass {
   ...
   */
  options(watch) {
    let sassOptions = ['--precision=8', '--output-style=' + (Mix.inProduction() ? 'compressed' : 'expanded')]

    if (watch) sassOptions.push('--watch')

    if (this.pluginOptions.includePaths) {
      this.pluginOptions.includePaths.forEach(path => sassOptions.push('--include-path=' + path))
    }
